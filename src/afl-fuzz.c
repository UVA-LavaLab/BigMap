...
static void usage(u8* argv0) {
  ...
  SAYF(
      ...
      "BigMap settings:\n"
      "  -K            - Disable shared memory allocation on hugepage.\n"
      "                  Enabled by default (see Readme for prerequisite).\n\n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv,
                       "+c:i:I:o:f:m:t:T:dnCB:S:M:x:QNUWKe:p:s:V:E:L:hRP:")) > 0)
    ...
    switch (opt) {
      ...
      case 'K':
        ...
        disable_hugepage = 1;
        WARNF("Disabled hugepage. Performance might not be optimal.\n");
        break;
